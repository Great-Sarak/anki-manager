...
Listens on a configurable port. Returns canned JSON responses for the
actions the bootstrap script uses (`version`, `getActiveProfile`,
`importPackageWithLog`). Each test programs responses via a JSON file
passed via --responses; the file maps action name -> response object
(or to a list-queue of objects consumed in order).

Logs every received request as one JSON line to --log (if given) so
tests can assert that the script made the calls in the expected order
with the expected params.

Prints "READY <port>" to stdout when bound. Runs until SIGTERM/SIGINT.
"""
